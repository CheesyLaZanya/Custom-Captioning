...
def load_pixel_values(image, input_size=448, max_num=6):
...
def generate_caption(model: AutoModel, processor: AutoTokenizer, image: Image.Image, prompt: str) -> str:
    ...
    pixel_values = load_pixel_values(image, max_num=6).to(torch.bfloat16).to(device)
...
def get_suggested_models() -> List[str]:
    suggested_models = [
        ...
        # 'OpenGVLab/InternVL2-4B', # Requires an older version of the transformer libray (4.37.2)?
        # Otherwise recieve Error: `shape '[-1, 0]' is invalid for input of size 1871`
        # 'OpenGVLab/InternVL2-8B' # Requires more GPU Memory
    ]
